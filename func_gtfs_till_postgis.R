...
#========== Skapa uppkoppling ===========
# Funktion för att koppla upp mot databasen. Kan användas med defaultvärden enligt nedan eller egna parametrar.
...
uppkoppling_db <- function(
  ...
  tryCatch({
    ...
    con <- dbConnect(          
      RPostgres::Postgres(),
      bigint = "integer",  
      user = key_list(service = keyring_service_name)$username,
      password = key_get(keyring_service_name, key_list(service = keyring_service_name)$username),
      host = db_host,
      port = db_port,
      dbname = db_name,
      options=db_options)
    ...
    stop(paste("Ett fel inträffade vid anslutning till databasen:", e$message))
  })
